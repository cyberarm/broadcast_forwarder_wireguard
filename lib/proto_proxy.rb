class BroadcastForwarderWireGuard
  class ProtoProxy
    ...
    def initialize(config_file: "#{ROOT_PATH}/config/config.json", gui: nil)
      @gui = gui
      ...
      log "Will deliver remote broadcasts to real interface: #{@config["local_lan_interface"]}, sent to vpn interface: #{@config["local_vpn_interface"]}"
    ...
    def local_broadcast_to_interface(message, socket)
      ...
      socket_exist = @sock
      @sock ||= UDPSocket.new
      @sock.bind("local_lan_interface", 0) unless socket_exist

      @total_tx += data.length
      @sock.send(data, 0, @config["local_lan_interface"], addr[1])
    end
